 ...
 * macWrappers/End.jsx
 *
 * Vendored copy of components/organisms/Wrappers/End.jsx
 * (which lives in the read-only emudeck-gui-components submodule).
 *
 * Change vs. upstream:
 *   - checkInstallation() now skips while disabledNext === true.
 *     On macOS, setup.sh runs inside a Terminal window (fire-and-forget
 *     AppleScript) so the `finish` IPC event could fire before all
 *     emulators are installed, causing the Post-Installation Status UI
 *     to appear prematurely with stale "all failed" data.
 *     The sentinel-file fix in EndPage.jsx already prevents this race,
...
function End({ message, percentage, step, disabledNext }) {
  ...
  // We check if everything installed — only AFTER the installer has actually
  // finished (disabledNext flips to false). Skipping while disabledNext===true
  // prevents the Post-Installation Status UI from showing stale "all failed"
  // data on macOS, where setup.sh runs asynchronously inside a Terminal window.
  useEffect(() => {
    if (system === 'win32' || disabledNext) return;
    checkInstallation();
  }, [disabledNext]);
